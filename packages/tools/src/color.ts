...
export function rgbToHex(_color: string) {
  const arr = _color.replace('rgb(', '').replace(')', '').split(',').map((i) => Number(i));
  const r = +arr[0];
  const g = +arr[1];
  const b = +arr[2];
...
function colorDark(hex: string, lum: number) {
  const rgb = hexToRgb(hex).replace('rgb(', '').replace(')', '').split(',');
  lum = lum || 0;
  for (let i = 0; i < 3; i++) rgb[i] = Math.floor(Number(rgb[i]) * (1 - lum)) as any;
